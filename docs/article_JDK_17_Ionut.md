...
Looking at the assemnly generated by the method to be optimized (e.g., `method_call_dse()`) we can summarize:

- OpenJdk HotSpot C2 does inlining of all the methods, nevertheless, it cannot completly remove the loops pertaining to the dead store method calls.
...
```
Then, the main loop that matters for the returning result it is unrolled by a factor of 32.

- GraalVM CE JIT has a similar behaviour as OpenJdk HotSpot C2 JIT, nevertheless, with an important difference: it does not do any loop unrolling, this is why it is the slowest
...
- GraalVM EE JIT complently removes dead code and the main loop is unrolled by a factor of 16.

...
Both GraalVM CE/EE JIT performs better than OpenJDK HotSpot C2 JIT. 

A matter of particular interest is, for example, the `branch_escape_obj` benchmark where OpenJDK HotSpot C2 JIT is one order of magnitude slower.
...
Looking at the generated assembly code generated by the OpenJDK HotSpot C2 JIT we can see the first object is allocated (since it is declared at the beginning of the method) and then the comparison is made and the result is returned. 
Nevertheless, since the comparison always evaluates to false, the previously allocated object is useless.

```
                    ...
                    <--- allocates the wrapper object -->
                    ... 
  0x7ffb58f6315a:   mov    0x10(%rsp),%r10
  0x7ffb58f6315f:   movzbl 0x14(%r10),%ebp      ;*getfield objectEscapes
  0x7ffb58f63164:   test   %ebp,%ebp
  0x7ffb58f63166:   jne    0x7ffb58f631aa       ;*ifeq
  0x7ffb58f63168:   mov    0x18(%r10),%r10d
  0x7ffb58f6316c:   mov    %r10,%rax
  0x7ffb58f6316f:   shl    $0x3,%rax            ;*getfield CACHED_WRAPPER
  0x7ffb58f63185:   ret
```
...
```
  ...
  0x7fe836b18d34:   jne    0x7fe836b18d64           ;*ifne allocates
  0x7fe836b18d3a:   nopw   0x0(%rax,%rax,1)
  0x7fe836b18d40:   cmpl   $0x0,0x10(%rsi)
  0x7fe836b18d44:   jl     0x7fe836b18d86           ;*newarray 
  0x7fe836b18d4a:   mov    0x18(%rsi),%eax
  0x7fe836b18d4d:   shl    $0x3,%rax                ;*getfield CACHED_WRAPPER 
  0x7fe836b18d63:   ret                             ;*areturn the cached wrapper
```

The same optimization happens in GraalVM CE JIT, which explains why this benchmark is much better in GraalVM.

In the case of `arg_escape_obj` benchmark:
...
where OpenJDK HotSpot C2 JIT is (again) one order of magnitude slower, GraalVM CE/EE JIT succeeds to optimize everything and returns just the constant:

```
  0x7f662eb1d030:   cmpl   $0x0,0x10(%rsi)
  0x7f662eb1d034:   jl     0x7f662eb1d052           ;*ifl allocates
  0x7f662eb1d03a:   mov    $0x1,%eax                ;*ireturn the constant true
  0x7f662eb1d051:   ret
```

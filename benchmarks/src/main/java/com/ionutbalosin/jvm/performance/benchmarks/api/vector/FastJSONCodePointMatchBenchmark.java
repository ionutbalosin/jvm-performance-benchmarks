...
public class FastJSONCodePointMatchBenchmark {
  ...
  // With new Vector.selectFrom API we can declare 128 byte lookup tables.
  ...
  public long match_avx2() {
    ...
    for (int i = 0; i < B128.loopBound(str.length); i += B128.length()) {
      ...
      VectorShuffle<Byte> index_lo = vsrc.lanewise(VectorOperators.AND, 0xF).toShuffle();
      VectorShuffle<Byte> index_hi = vsrc.lanewise(VectorOperators.LSHR, 4).toShuffle();
      ByteVector lookup_lut1 =
          lut1_lo.rearrange(index_lo).lanewise(VectorOperators.AND, lut1_hi.rearrange(index_hi));
      matches += lookup_lut1.compare(VectorOperators.GT, 0).and(index_mask).trueCount();

      ByteVector lookup_lut2 =
          lut2_lo.rearrange(index_lo).lanewise(VectorOperators.AND, lut2_hi.rearrange(index_hi));
      matches += lookup_lut2.compare(VectorOperators.GT, 0).and(index_mask).trueCount();

      ByteVector lookup_lut3 =
          lut3_lo.rearrange(index_lo).lanewise(VectorOperators.AND, lut3_hi.rearrange(index_hi));
      matches += lookup_lut3.compare(VectorOperators.GT, 0).and(index_mask).trueCount();

      ByteVector lookup_lut4 =
          lut4_lo.rearrange(index_lo).lanewise(VectorOperators.AND, lut4_hi.rearrange(index_hi));
      matches += lookup_lut4.compare(VectorOperators.GT, 0).and(index_mask).trueCount();

      ByteVector lookup_lut5 =
          lut5_lo.rearrange(index_lo).lanewise(VectorOperators.AND, lut5_hi.rearrange(index_hi));
